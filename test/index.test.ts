...
describe("parse-full-name", function () {
  describe("parseFullName", function () {
    ...
    it("parses nicknames", function () {
      ...
      verifyName(
        parseFullName("Simpson, Orenthal James “O. J.”"),
        ["", "Orenthal", "James", "Simpson", "O. J.", ""],
        []
      );
      verifyName(
        parseFullName("Simpson, Orenthal ‘O. J.’ James"),
        ["", "Orenthal", "James", "Simpson", "O. J.", ""],
        []
      );
      verifyName(
    ...
    it("parses known suffixes", function () {
      ...
      verifyName(
        parseFullName("dipl.-ing. Hans Zimmer", "all", 1),
    ...
    it("manually fixes case, or not, when specified", function () {
      ...
      verifyName(
        parseFullName("Mr. JÜAN MARTINEZ (MARTIN) DE LORENZO Y GUTIEREZ JR.", "all", 1),
        ["Mr.", "Jüan", "Martinez", "de Lorenzo y Gutierez", "Martin", "Jr."],
        []
      );
      verifyName(
        parseFullName("Dr. b.a. Julia Storm", "all", 1),
        ["Dr.", "Julia", "", "Storm", "", "B.A."],
        []
      );
      verifyName(
        parseFullName("mr. jüan martinez (martin) de lorenzo y gutierez jr.", "all", 0),
        ["mr.", "jüan", "martinez", "de lorenzo y gutierez", "martin", "jr."],
        []
      );
    });
    ...
    it("returns a single part, when specified", function () {
      expect(parseFullName("Mr. Jüan Martinez (Martin) de Lorenzo y Gutierez Jr.", "title")).toBe(
        "Mr."
      );
      expect(parseFullName("Mr. Jüan Martinez (Martin) de Lorenzo y Gutierez Jr.", "first")).toBe(
        "Jüan"
      );
      expect(parseFullName("Mr. Jüan Martinez (Martin) de Lorenzo y Gutierez Jr.", "middle")).toBe(
        "Martinez"
      );
      expect(parseFullName("Mr. Jüan Martinez (Martin) de Lorenzo y Gutierez Jr.", "last")).toBe(
        "de Lorenzo y Gutierez"
      );
      expect(parseFullName("Mr. Jüan Martinez (Martin) de Lorenzo y Gutierez Jr.", "nick")).toBe(
        "Martin"
      );
      expect(parseFullName("Mr. Jüan Martinez (Martin) de Lorenzo y Gutierez Jr.", "suffix")).toBe(
        "Jr."
      );
    });
